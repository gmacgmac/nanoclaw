/**
 * Command Approval — Dangerous Command Detector
 *
 ...
 *
 * Used by: nanoclaw MCP server (BE_05, copied to container/agent-runner/src/lib/)
 * Source of truth: src/lib/command-approval.ts
 */
...
/**
 * Assess whether a command matches any dangerous patterns.
 */
...
/**
 * Determine whether a command requires user approval.
 *
 * Approval is needed when:
 * 1. The command is dangerous (matches a pattern), AND
 * 2. The command references a write-mounted path
 *
 * Commands targeting only container-internal paths do NOT require approval —
 * the container itself is the security boundary.
 *
 * @param command - The shell command to evaluate
 * @param writeMountPaths - Container paths that map to real host data
 *   (from additionalMounts where readonly !== true, always under /workspace/extra/)
 */
export function requiresApproval(
  ...
  // Check if the command references any write-mounted path
