 ...
 *
 * Used by: nanoclaw-web-search MCP server, brave-search MCP server (BE_02)
 */
...
function expandMappedIpv4(lower: string): string | null {
  // Matches ::ffff:XXXX:XXXX (hex groups, as Node normalises them)
  ...
  // Also handle dotted-decimal form ::ffff:x.x.x.x (user-supplied URLs)
 ...
 * Handles loopback (::1) and IPv6-mapped IPv4 addresses (both hex and dotted forms).
 * Input may include surrounding brackets (as returned by URL.hostname).
 */
function isBlockedIpv6(ip: string): string | null {
  ...
  // IPv6-mapped IPv4 — check the embedded IPv4 address
 ...
 * Returns the label of the matched range, or null if allowed.
 */
