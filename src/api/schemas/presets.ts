...
export const ModelCapabilitiesSchema = z.object({
  ...
  thinking: z.boolean().optional().describe('Extended thinking / reasoning mode.'),
  tools: z.boolean().optional().describe('Native tool/function calling support.'),
  nativeWebTools: z.boolean().optional().describe(
...
export const ModelPresetInputSchema = z.object({
  ...
  compactThreshold: z.number().min(0.1).max(0.95).optional()
    .describe('Fraction of context window at which to trigger compaction. Range: 0.1–0.95.'),
  webSearchVendor: z.string().optional()
    .describe('Web search vendor key. Default: "ollama".'),
  transform: z.enum(['openai']).optional()
    .describe('Request transform to apply. Currently only "openai" is supported.'),
  sdkMode: z.enum(['anthropic', 'bedrock']).optional()
