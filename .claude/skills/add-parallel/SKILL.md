...
Verify the build:
```bash
echo '{}' | docker run -i --entrypoint /bin/echo nanoclaw-agent:stable "Container OK"
```
...
- Verify scheduled task was created:
  ```bash
  curl -sS -H "Authorization: Bearer $API_TOKEN" http://localhost:3100/api/scheduled-tasks | jq .
  ```
