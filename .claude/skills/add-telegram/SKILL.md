...
```bash
TELEGRAM_CHOC_BOT_TOKEN=REDACTED_TOKEN
```
...
```bash
npx tsx setup/index.ts --step register -- --jid "tg:123456789:choc" --name "GM Choc" --folder "choc_main" --trigger "@${ASSISTANT_NAME}" --channel telegram --endpoint <provider> --bot-token-name choc
```
...
Then clean up the Message Formatting section: the template contains formatting rules for all channels (Telegram, Slack, WhatsApp, Discord). Since this is a Telegram group, **remove all non-Telegram formatting subsections** from the copied CLAUDE.md. Keep only the Telegram block. Also add a line at the top of the Message Formatting section: "All output goes to Telegram. Format accordingly."
...
> **Command menu:** The Telegram `/` command menu auto-syncs whenever a group's `allowedHostCommands` changes at runtime. No restart required.
