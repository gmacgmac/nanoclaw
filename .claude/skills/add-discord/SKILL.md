...
```bash
npx tsx setup/index.ts --step register -- --jid "dc:<channel-id>" --name "<server-name> #<channel-name>" --folder "discord_main" --trigger "@${ASSISTANT_NAME}" --channel discord --endpoint <provider> --no-trigger-required --is-main
```

For additional channels (trigger-only):

```bash
npx tsx setup/index.ts --step register -- --jid "dc:<channel-id>" --name "<server-name> #<channel-name>" --folder "discord_<channel-name>" --trigger "@${ASSISTANT_NAME}" --channel discord --endpoint <provider>
```
...
Then clean up the Message Formatting section: the template contains formatting rules for all channels (Telegram, Slack, WhatsApp, Discord). Since this is a Discord group, **remove all non-Discord formatting subsections** from the copied CLAUDE.md. Keep only the Discord block. Also add a line at the top of the Message Formatting section: "All output goes to Discord. Format accordingly."
